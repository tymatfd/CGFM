...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.extend_layer = nn.Conv2d(64,256,kernel_size=1, padding=0, bias=False)
        embed_dims = 128

        
        

        self.atts_gen_layer = nn.Sequential(
                nn.Conv2d(embed_dims , embed_dims//16, kernel_size=1, stride=1, padding=0),
                nn.ReLU(),
                nn.Conv2d(embed_dims//16, embed_dims//2, kernel_size=1, stride=1, padding=0),
                #nn.Sigmoid()
                )
        self.attp_post = nn.Sequential(
            nn.Conv2d(embed_dims//2, embed_dims//2, kernel_size=1, stride=1, padding=0),
            nn.ReLU(),
            
                        
        )
        self.atts_post = nn.Sequential(
            nn.Conv2d(embed_dims//2, embed_dims//2, kernel_size=1, stride=1, padding=0),
            nn.ReLU()
    ...
    def forward(self, x, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        mask_ =  F.interpolate(mask, size=(supp_feat_3.size(2),supp_feat_3.size(3)), mode='bilinear', align_corners=True)
        ...
        if self.shot == 1:
            #similarity3 = get_similarity(query_feat, supp_feat, s_y)
        ...
        supp_mgap5 = Weighted_GAP(supp_feat_5, F.interpolate(mask, size=(supp_feat_5.size(2), supp_feat_5.size(3)), mode='bilinear', align_corners=True))
        supp_mgap4 = Weighted_GAP(supp_feat_4, F.interpolate(mask, size=(supp_feat_4.size(2), supp_feat_4.size(3)), mode='bilinear', align_corners=True))
        similarity1s = F.cosine_similarity(supp_feat_5,supp_mgap5).unsqueeze(1)
        similarity2s = F.cosine_similarity(supp_feat_4,supp_mgap4).unsqueeze(1)
        similaritys = torch.cat([similarity1s, similarity2s], dim=1)
        ...
        meta_map_bg = F.interpolate(meta_map_bg, size=(base_map.shape[-2],base_map.shape[-1]), mode='bilinear', align_corners=True)

        merge_map = torch.cat([meta_map_bg, base_map], 1)
        merge_bg = self.cls_merge(merge_map)
        meta_map_fg = F.interpolate(meta_map_fg, size=(base_map.shape[-2],base_map.shape[-1]), mode='bilinear', align_corners=True)                     # [bs, 1, 60, 60]
        ...
        if self.training:
            ...
            return final_out.max(1)[1], main_loss, distil_loss / 3  , aux_loss2#+ pre_loss
        else:
    ...
    def get_optim(self, model, args, LR):
        ...
        [
            ...
            {'params': model.atts_post.parameters(), "lr": LR*10},
            {'params': model.attp_post.parameters(), "lr": LR*10},
            #{'params': model.att_gen_layer.parameters(), "lr": LR*10},
            {'params': model.atts_gen_layer.parameters(), "lr": LR*10},
            #{'params': model.coef_gen_layer.parameters(), "lr": LR*10},
            #{'params': model.gate_post.parameters(), "lr": LR*10},
            #{'params': model.pre_cls.parameters(), "lr": LR*10},
